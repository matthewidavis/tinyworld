...
DEBUG = True  # If True, we print debug logs (corners, etc.)
...
# Output canvas resolution for partial panorama
...
def tile_stamp(
    ...
    """
    Stamp `img` onto the partial equirectangular canvas (pano_map)
    based on a tile approach:
      - The tile covers [center_pan - hfov/2, center_pan + hfov/2] in pan, and
        [center_tilt - vfov/2, center_tilt + vfov/2] in tilt.
    We skip pixels outside the partial range (pan_min..pan_max, tilt_min..tilt_max).
    Overlapping areas are alpha-blended 50/50.

    Vertical mapping is defined so that:
      - tilt = TILT_MAX (+90°) maps to y = 0 (top)
    ...
    for y_img in range(h):
        ...
        if lat_deg < tilt_min or lat_deg > tilt_max:
            continue
        # Map vertical: tilt_max maps to y = 0, tilt_min to y = pano_h
...
def partial_stitch_images(
    ...
    Build a partial equirectangular panorama covering:
       Pan: -135..+135 (270°) and Tilt: -30..+90 (120°)
    ...
    for idx, (img, meta) in enumerate(zip(image_list, freeD_data_list)):
        ...
        # Flip the image vertically to correct individual tile orientation.
...
def main_capture():
    """
    Capture snapshots over pan=-135..+135 and tilt=-30..+90, then
    stitch them into a partial equirectangular (270°x120°) panorama
    using a tile-based approach.
    """
    ...
    panorama = partial_stitch_images(
        captured_images,
        metadata_list,
        out_width=OUT_WIDTH,
        out_height=OUT_HEIGHT,
    ...
    cv2.imwrite("partial_panorama.jpg", panorama)
    cv2.imshow("Partial Panorama", panorama)
    cv2.waitKey(0)
