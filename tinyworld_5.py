...
def tile_stamp(
    ...
    for y_img in range(h):
        ...
        if lat_deg < TILT_MIN or lat_deg > TILT_MAX:
            continue
        # partial equirect: top=+90 => y=0, bottom=-30 => y=pano_h
...
def place_partial_in_full_equirect(partial_pano):
    ...
    for py in range(partial_h):
        # lat in [+90..-30]
        # top row => lat=+90 => py=0
        # bottom => lat=-30 => py=partial_h
        lat_frac = py / (partial_h - 1)  # [0..1]
        lat_deg = TILT_MAX - lat_frac * (TILT_MAX - TILT_MIN)  # from +90..-30

        # map lat_deg => full domain
        # full domain: lat in [+90..-90], height=FULL_H
        if lat_deg < -90 or lat_deg > 90:
            continue
        # lat_deg_frac = (lat_deg - (-90)) / (180) = (lat_deg + 90)/180
        ...
        for px in range(partial_w):
            ...
            if lon_deg < -180 or lon_deg > 180:
                continue
            # map lon_deg => full domain [-180..+180]
            # fraction = (lon_deg + 180)/360 => [0..1]
...
def main_capture():
    """
    Capture partial coverage in [-135..+135, -30..+90], build partial equirect,
    then place it in a full 360x180 2:1 canvas, leaving the rest black.
    """
    ...
    # 1) Build partial equirect (270x120 coverage)
    partial_pano = partial_stitch_images(captured_images, metadata_list,
    ...
    # 2) Place partial coverage into a full 360x180 equirect
